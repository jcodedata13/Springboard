...
SELECT DISTINCT CONCAT(m.surname,', ',m.firstname) AS name,f.name AS court
FROM Members AS m
...
SELECT CONCAT(m.surname,', ',m.firstname) AS name, f.name, f.membercost, f.guestcost
FROM Members AS m
INNER JOIN Bookings AS b
ON m.memid = b.memid
INNER JOIN Facilities as f
ON f.facid = b.facid
WHERE (f.membercost + f.guestcost) > 30 AND starttime LIKE "2012-09-14%"
...
SELECT name,facility,membercost,guestcost
FROM (SELECT CONCAT(m.surname,', ',m.firstname) AS name, f.name AS facility, f.membercost AS membercost, f.guestcost AS guestcost
      FROM Members AS m
      INNER JOIN Bookings AS b
	  ON m.memid = b.memid	
	  INNER JOIN Facilities AS f
